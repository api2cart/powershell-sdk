#
# API2Cart OpenAPI
# API2Cart
# Version: 1.1
# Contact: contact@api2cart.com
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

No description available.

.PARAMETER ProductId
Defines the product where the price has to be updated
.PARAMETER GroupPrices
Defines product's group prices
.OUTPUTS

ProductPriceUpdate<PSCustomObject>
#>

function Initialize-ProductPriceUpdate {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [String]
        ${ProductId},
        [Parameter(Position = 1, ValueFromPipelineByPropertyName = $true)]
        [PSCustomObject[]]
        ${GroupPrices}
    )

    Process {
        'Creating PSCustomObject: PSOpenAPITools => ProductPriceUpdate' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug


        $PSO = [PSCustomObject]@{
            "product_id" = ${ProductId}
            "group_prices" = ${GroupPrices}
        }


        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to ProductPriceUpdate<PSCustomObject>

.DESCRIPTION

Convert from JSON to ProductPriceUpdate<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

ProductPriceUpdate<PSCustomObject>
#>
function ConvertFrom-JsonToProductPriceUpdate {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => ProductPriceUpdate' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in ProductPriceUpdate
        $AllProperties = ("product_id", "group_prices")
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "product_id"))) { #optional property not found
            $ProductId = $null
        } else {
            $ProductId = $JsonParameters.PSobject.Properties["product_id"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "group_prices"))) { #optional property not found
            $GroupPrices = $null
        } else {
            $GroupPrices = $JsonParameters.PSobject.Properties["group_prices"].value
        }

        $PSO = [PSCustomObject]@{
            "product_id" = ${ProductId}
            "group_prices" = ${GroupPrices}
        }

        return $PSO
    }

}

